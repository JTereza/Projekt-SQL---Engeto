...
SELECT DISTINCT 
	...
	cp1.payroll_year,
	cp2.celk_avg_mzdy,
	round(cp2.celk_avg_mzdy/cp1.average_potraviny,0) AS result
FROM t_tereza_jurakova_project_sql_primary_final AS cp1
JOIN 
	(SELECT 
		payroll_year, 
		round(avg(average_mzdy),2)  AS celk_avg_mzdy
	FROM t_tereza_jurakova_project_sql_primary_final
	WHERE 
		payroll_year IN ('2006')
	GROUP BY 
